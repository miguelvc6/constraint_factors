...
- **Outputs:** Graph artifacts in `data/processed/<variant>/` (`{split}_graph-<encoding>.pkl` or sharded `.pt/.pkl` files), per-split manifests (`{split}_graph-<encoding>.pkl.manifest.json`), `target_vocabs.json`, plus optional visualisations like `graph_visualization.png`.
