...
| Evidence state           | Focus/comparison statements could be absent, a later entity snapshot could already contain a gold addition or omit a gold deletion, shared-subject comparison rows could attach the third entity description to the wrong entity, and evaluation could discard the declared edit before reconstructing PRE. | Focus/comparison statements are seeded, the third description is assigned to `other_object` when the subject is shared and to `other_subject` otherwise, then the declared transition is reversed to construct PRE. Global evaluation retains those add/delete columns and requires recomputed PRE to match authoritative factor labels on every strict-evaluation row. POST_GOLD reapplies the edit. |
...
| Gold leakage             | Candidate inference and oracle analysis used a builder that could inspect `graph.y`.                                                                                                                                                  | Training and inference APIs are separate. `build_inference_candidates` has no graph/gold parameter; all evaluation/oracle consumers use it. Empty inference pools produce one visible `fallback_noop`; reranker validation never force-includes gold.                                                                                              |
...
| Artifact provenance      | Filenames did not prove config/data/code compatibility.                                                                                                                                                                               | Graph schema v3 commits to full input/shard hashes, sizes, counts, modes, and derivation lineage. Checkpoints embed config hashes and seed. Run/evaluation manifests bind those artifacts; evaluation rejects config/checkpoint mismatch.                                                                                                           |
| Acceptance gate          | Stale or partially supported results could be reused without a machine check.                                                                                                                                                          | `scripts/validate_scientific_integrity.py` validates schema-v2 data, primary invariants, graph batching, A1 sharing, checkpoint compatibility/provenance, strict identity metrics, pooled ratios, full primary-denominator support, and the all-row PRE-state parity audit.                                                                       |
...
| Factorized and passive graphs                        | Rebuild                            | Identity-keyed nodes, schema-v3 lineage/full hashes, corrected batching.      |
...
```bash
uv run src/02_dataframe_builder.py --dataset full --output-dataset full --min-occurrence 100 --split-policy preserve --overwrite && uv run src/03_constraint_registry.py --dataset full && uv run src/05_constraint_labeler.py --dataset full --output-dataset full_valid --min-occurrence 100 --registry-dataset full --constraint-scope local --factor-family-policy supported_only --filter-invalid-primary --overwrite && uv run src/02b_stratified_benchmark_sampler.py --source-dataset full_valid --output-dataset full_strat1m --min-occurrence 100 --target-rows 1000000 --seed 42 --scope local --overwrite
```
...
```bash
uv run scripts/validate_scientific_integrity.py --dataset-variant full_strat1m_minocc100 --stage interim --verify-hashes --output models/paper_diagnostics/integrity_preflight_interim.json
```
...
```bash
uv run src/05_constraint_labeler.py --dataset full --output-dataset full_valid --min-occurrence 100 --registry-dataset full --constraint-scope local --factor-family-policy supported_only --filter-invalid-primary --overwrite && uv run src/02b_stratified_benchmark_sampler.py --source-dataset full_valid --output-dataset full_strat1m --min-occurrence 100 --target-rows 1000000 --seed 42 --scope local --overwrite
```

### 2. Graphs

```bash
uv run src/06_graph.py --dataset full_strat1m --min-occurrence 100 --encoding node_id --constraint-scope local --constraint-representation factorized --primary-constraint-mode executable_factor --registry-dataset full --shard-size 200000 --use-torch-save --overwrite atomic && uv run src/06_graph.py --dataset full_strat1m --min-occurrence 100 --encoding node_id --constraint-representation eswc_passive --registry-dataset full --shard-size 200000 --use-torch-save --overwrite atomic && uv run scripts/validate_scientific_integrity.py --dataset-variant full_strat1m_minocc100 --encoding node_id --constraint-representation factorized --stage data --verify-hashes --output models/paper_diagnostics/integrity_preflight_factorized.json && uv run scripts/validate_scientific_integrity.py --dataset-variant full_strat1m_minocc100 --encoding node_id --constraint-representation eswc_passive --stage data --verify-hashes --output models/paper_diagnostics/integrity_preflight_passive.json
```
...
```bash
uv run scripts/make_experiment_configs.py --variant full_strat1m_minocc100 --encoding node_id && uv run scripts/make_experiment_configs.py --variant full_strat1m_minocc100 --encoding node_id --include-h2-ablations
```

### 5. Baselines and learned suite

```bash
uv run src/09_eval.py --run-baselines --dataset full_strat1m --min-occurrence 100 --registry-dataset full --strict-global-metrics --per-constraint-csv && uv run src/10_scheduler.py --only b0_eswc_reproduction__full_strat1m_minocc100__node_id --paper-suite --seed 42 && uv run src/10_scheduler.py --only a1_factorized_imitation__full_strat1m_minocc100__node_id --paper-suite --seed 42 && uv run src/10_scheduler.py --only m1c_safe_factor_chooser__full_strat1m_minocc100__node_id --paper-suite --seed 42 && uv run src/10_scheduler.py --only m1d_safe_factor_direct__full_strat1m_minocc100__node_id --paper-suite --seed 42 && uv run src/10_scheduler.py --only g0_globalfix_reference__full_strat1m_minocc100__node_id --paper-suite --seed 42
```
...
- Graph manifests are schema v3, exact artifact/hash/count checks pass, and no incomplete marker remains.
...
- Evaluation and reranker histories report fallback no-op use; reranker validation uses inference candidates and records natural-gold coverage where applicable.
